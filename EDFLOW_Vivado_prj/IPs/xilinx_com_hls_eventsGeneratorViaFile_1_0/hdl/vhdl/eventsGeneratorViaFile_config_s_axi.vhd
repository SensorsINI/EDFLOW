...
entity eventsGeneratorViaFile_config_s_axi is
generic (
...
end entity eventsGeneratorViaFile_config_s_axi;
...
architecture behave of eventsGeneratorViaFile_config_s_axi is
    ...
    signal int_input_V_ap_vld  : STD_LOGIC := '0';
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INPUT_V_CTRL =>
                        rdata_data <= (0 => int_input_V_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    input_V_ap_vld       <= int_input_V_ap_vld;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_V_ap_vld <= '0';
            elsif (ACLK_EN = '1') then
                if (w_hs  = '1' and waddr = ADDR_INPUT_V_CTRL and WSTRB(0) = '1' and WDATA(0) = '1') then
                    int_input_V_ap_vld <= '1';
                else
                    int_input_V_ap_vld <= '0'; -- self clear
                end if;
            end if;
        end if;
    end process;
